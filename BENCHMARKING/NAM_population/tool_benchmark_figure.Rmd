...
str(results_df)
print(results_df)
results_df['unweighted_score'] = (results_df['precision'] + results_df['sensitivity'] +results_df['F1'] + results_df['runtime'] + results_df['memory'] + results_df['FDR'] + results_df['power_of_discovery'])/7
# Define weights
# Weighting factors
...
weight_power_of_discovery <- 0.1
weight_FDR <- 0.0  # No weight for FDR in this scenario

# Combine metrics into the final score with weighted contributions
results_df$weighted_score <- (weight_precision * results_df$precision) + 
                              (weight_runtime * results_df$runtime) + 
                              (weight_memory * results_df$memory) +
                              (weight_sensitivity * results_df$sensitivity) +
                              (weight_power_of_discovery * results_df$power_of_discovery) +
                              (weight_FDR * results_df$FDR)

# Find minimum and maximum scores
min_score <- min(results_df$weighted_score)
max_score <- max(results_df$weighted_score)

# Print weighted scores and dataframe
print(results_df$weighted_score)
print(results_df)

#############################################

# Define custom scaling functions
scale_to_01 <- function(x, ideal_value) {
  return(ifelse(ideal_value == 0, 0, x / ideal_value))
}

scale_to_10 <- function(x, ideal_value) {
  return(ifelse(ideal_value == 0, 0, 1 - (x / ideal_value)))
}

# Define ideal values for scaling
ideal_precision <- 1
ideal_sensitivity <- 1
ideal_F1 <- 1
ideal_power_of_discovery <- 1
ideal_FDR <- 1e-10  # A small non-zero value to prevent division by zero
ideal_runtime <- 1e-10  # A small non-zero value to prevent division by zero
ideal_memory <- 1e-10  # A small non-zero value to prevent division by zero

# Scale each metric
scaled_precision <- scale_to_01(results_df$precision, ideal_precision)
scaled_sensitivity <- scale_to_01(results_df$sensitivity, ideal_sensitivity)
scaled_F1 <- scale_to_01(results_df$F1, ideal_F1)
scaled_power_of_discovery <- scale_to_01(results_df$power_of_discovery, ideal_power_of_discovery)
scaled_FDR <- scale_to_10(results_df$FDR, ideal_FDR)
scaled_runtime <- scale_to_10(results_df$runtime, ideal_runtime)
scaled_memory <- scale_to_10(results_df$memory, ideal_memory)
...
# Combine scaled metrics into the final score with weighted contributions
results_df$weighted_score <- (weight_precision * scaled_precision) + 
                              (weight_runtime * scaled_runtime) + 
                              (weight_memory * scaled_memory) +
                              (weight_sensitivity * scaled_sensitivity) +
                              (weight_power_of_discovery * scaled_power_of_discovery) +
                              (weight_FDR * scaled_FDR)

# Find minimum and maximum scores
min_score <- min(results_df$weighted_score)
max_score <- max(results_df$weighted_score)

# Scale final scores to range [0, 1]
results_df$scaled_score <- (results_df$weighted_score - min_score) / (max_score - min_score)

# Print scaled scores and dataframe
print(results_df$scaled_score)
print(results_df)
...
combined_plot_update <- grid.arrange(plot1, plot4, plot2, plot5, ncol = 2)
